...
export const addToWishlist = asyncHandler(async (req, res, next) => {
  // try {
    ...
    console.log(bookName, email, "Book Name and Email in addToWishlist");
...
export const moveBookFromCartToWishlist = async (req, res, next) => {
  try {
    ...
    const bookInfo = await cartModel.findOneAndDelete({
      $and: [
        { bookName },
        { email }
      ]
...
export const deleteAllWishlistProduct =  asyncHandler(async (req, res, next) => {
  // try {
const email = req.gmail;
    if (!email) {
      return next(createHttpError(400, "Missing required field"))
